"""Evidence-only graph indexing and projection rebuild pipeline."""
...
def calculate_evidence_pageranks(
    ...
    """Calculate weighted PageRank without the removed graph library dependency."""

    nodes = tuple(dict.fromkeys(str(key) for key in entity_keys if str(key)))
    if not nodes:
        return {}
    node_set = set(nodes)
    outgoing: dict[str, dict[str, float]] = {node: {} for node in nodes}
    for relation in relations:
        from_key = str(relation.get("from_entity_key_kwd") or "")
        to_key = str(relation.get("to_entity_key_kwd") or "")
        if (
            not relation.get("relation_key_kwd")
            or from_key == to_key
            or from_key not in node_set
            or to_key not in node_set
        ):
            continue
        try:
            ...
            weight = 1.0
        outgoing[from_key][to_key] = outgoing[from_key].get(to_key, 0.0) + weight
        if not bool(relation.get("directed_int", True)):
            outgoing[to_key][from_key] = outgoing[to_key].get(from_key, 0.0) + weight

    count = len(nodes)
    damping = 0.85
    rank = {node: 1.0 / count for node in nodes}
    for _ in range(100):
        dangling = damping * sum(rank[node] for node in nodes if not outgoing[node]) / count
        updated = {node: (1.0 - damping) / count + dangling for node in nodes}
        for source, targets in outgoing.items():
            total = sum(targets.values())
            if not total:
                continue
            for target, weight in targets.items():
                updated[target] += damping * rank[source] * weight / total
        if sum(abs(updated[node] - rank[node]) for node in nodes) < count * 1e-6:
            rank = updated
            break
        rank = updated
    return rank
...
def _clean_keywords(values: Sequence[str]) -> tuple[str, ...]:
    return tuple(sorted({keyword for value in values if (keyword := _clean_display(value))}))
    ...
    for result in results:
        ...
        for entity in result.entities:
            ...
            if not display_name or not display_type:
                logger.warning(
                    "[EvidenceGraph] evidence_skipped kb_id=%s document_id=%s "
                    "reason=empty_entity_fields",
                    knowledge_id,
                    document_id,
                )
                continue
            normalized_key = entity_key(knowledge_id, display_name, display_type)
            ...
            aliases = tuple(
                dict.fromkeys(
                    alias for raw_alias in entity.aliases if (alias := _clean_display(raw_alias))
                )
            )
            description = _clean_display(entity.description)[:300]
            for raw_source_id in dict.fromkeys(entity.source_chunk_ids):
        ...
        for relation in result.relations:
            ...
            for raw_source_id in dict.fromkeys(relation.source_chunk_ids):
                source_id = str(raw_source_id).strip()
                if not source_id:
                    continue
...
class KnowledgeGraphIndexPipeline:
    ...
    async def sync_document(
        ...
        *,
        ...
        try:
            ...
            logger.error(
                "[EvidenceGraph] index_failed kb_id=%s document_id=%s stage=%s "
    ...
    async def _extract_batches(
        ...
        async def extract_one(batch: ExtractionBatch) -> ExtractionResult:
            async with semaphore:
                cache_key = self._extraction_cache.build_key(runtime, batch)
                ...
                await self._extraction_cache.set(cache_key, result)
                return result
    ...
    async def _rebuild_graph_pageranks(self, runtime: GraphIndexRuntime) -> None:
    ...
    def _most_common(values: Sequence[str] | Any) -> str:
        ...
        return max(ordered, key=lambda value: (counts[value], -ordered.index(value)))

    @staticmethod
    def _entity_degrees(evidence: Sequence[RelationEvidence]) -> dict[str, int]:
...
__all__ = [
    "KnowledgeGraphIndexPipeline",
    "calculate_evidence_pageranks",
