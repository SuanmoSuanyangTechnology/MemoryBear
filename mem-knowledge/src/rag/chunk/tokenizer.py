...
_RESOURCE_ROOT = Path(__file__).resolve().parents[1] / "res"
_TOKEN_DICTIONARY = _RESOURCE_ROOT / "huqie.txt"
_NLTK_DATA_ROOT = _RESOURCE_ROOT / "nltk_data"
...
class RagTokenizer:
    ...
    def loadDict_(self, fnm):
        ...
        with open(fnm, encoding="utf-8") as dictionary:
            for line in dictionary:
                ...
                k = self.key_(line[0])
                frequency = int(math.log(float(line[1]) / self.DENOMINATOR) + 0.5)
                if k not in self.trie_ or self.trie_[k][0] < frequency:
                    self.trie_[self.key_(line[0])] = (frequency, line[2])
                self.trie_[self.rkey_(line[0])] = 1

    def __init__(self, debug=False):
        nltk_data_root = str(_NLTK_DATA_ROOT)
        if nltk_data_root not in nltk.data.path:
        ...
        self.loadDict_(_TOKEN_DICTIONARY)
    ...
    def _strQ2B(self, ustring):
        ...
        for uchar in ustring:
            ...
            if inside_code == 0x3000:
                ...
                inside_code -= 0xFEE0
            # Preserve the original character if conversion did not yield half-width text.
    ...
    def english_normalize_(self, tks):
        return [
            self.stemmer.stem(self.lemmatizer.lemmatize(t)) if re.match(r"[a-zA-Z_-]+$", t) else t
            for t in tks
        ]
    ...
    def tokenize(self, line):
        ...
        for L, lang in arr:
            ...
            while i < len(tks1) and j < len(tks):
                ...
                same = 1
                while (
                    i + same < len(tks1) and j + same < len(tks) and tks1[i + same] == tks[j + same]
                ):
                    same += 1
                res.append(" ".join(tks[j : j + same]))
                _i = i + same
...
def get_tokenizer() -> RagTokenizer:
    """Build the legacy-compatible tokenizer on first token use."""

    return RagTokenizer()


def tokenize(value: str) -> str:
    return get_tokenizer().tokenize(value)


def fine_grained_tokenize(value: str) -> str:
    return get_tokenizer().fine_grained_tokenize(value)


__all__ = ["RagTokenizer", "fine_grained_tokenize", "get_tokenizer", "tokenize"]
