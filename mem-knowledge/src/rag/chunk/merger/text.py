...
class TextMerger:
    ...
    def _split_recursive(
        ...
        separator = (
            self.separators[separator_index] if separator_index < len(self.separators) else ""
        )
        if separator == "":
    ...
    def _merge_split_units(
        ...
        for split_unit in split_units:
            if current_doc and not self._within_limit(
                self._join_units([*current_doc, split_unit]), limit
            ):
    ...
    def _hard_split(self, text: str, limit: int) -> list[str]:
        ...
        while start < len(tokens):
            ...
            if boundary is None:
                raise RuntimeError(
                    f"Unable to find a valid UTF-8 boundary from token index {start}."
                )
