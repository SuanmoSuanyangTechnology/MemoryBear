...
from ..parser.markdown_preprocessor import (
    ALPHA_LIST_PATTERN,
...
from ..token_utils import get_encoder, num_tokens_from_string
from .base import ChunkMerger
from .structured_stream import (
    BlockSpan,
    SourceFragment,
    StructuredStream,
    block_separator,
...
class BlockMerger(ChunkMerger):
    ...
    def _merge_structured_stream(self, ctx: ChunkContext, parse_result: ParseResult) -> MergeResult:
        ...
        if ctx.chunk_output_mode is ChunkOutputMode.PARENT_CHILD:
            if ctx.parser_config.get("parent_chunk_mode") == "full-doc":
                parent_drafts = self._build_full_doc_parent_drafts(parse_result.blocks or [])
            else:
    ...
    def _normalize_table_child_parts(child_drafts: list[_ChildDraft]) -> None:
        ...
        for child_draft in child_drafts:
            if (
                ...
                continue
            parts_by_source.setdefault(child_draft.source_key, []).append(child_draft.chunk)
    ...
    def _specialized_children_from_fragment(
        ...
        return [self._draft_to_normal_chunk(self._units_to_draft([unit])) for unit in units]
    ...
    def _code_children_from_fragment(
        ...
        if any(num_tokens_from_string(piece) > token_num for piece in pieces):
            raise ValueError(f"Structured wrapper cannot fit within token limit {token_num}.")
        return [
            LogicalChunk(
                type=LogicalChunkType.TEXT,
                content=piece,
    ...
    def _split_stream_unit(self, unit: _StreamUnit, token_num: int) -> list[_StreamUnit]:
        ...
        if block_type not in {
            ...
            return [
                _StreamUnit(
                    ...
                    separator_before=(unit.separator_before if index == 0 else text_unit.prefix),
                )
        ...
        total = len(pieces)
        if any(num_tokens_from_string(piece) > token_num for piece in pieces):
            raise ValueError(f"Structured content cannot fit within token limit {token_num}.")
    ...
    def _draft_to_normal_chunk(self, draft: _ChunkDraft) -> LogicalChunk:
        ...
        if single_source and block.type is ParsedBlockType.IMAGE:
            image_tag_complete = len(draft.fragments) == 1 and draft.fragments[0].complete
            metadata = self._metadata_for_block(block)
    ...
    def _draft_fragments(units: list[_StreamUnit]) -> list[SourceFragment]:
        ...
        for unit in units:
            ...
            if (
                ...
                and fragment.block.type in {*TEXT_LIKE_TYPES, ParsedBlockType.LIST}
            ):
                ...
                fragments[-1] = SourceFragment(
                    ...
                    structure_valid=(previous.structure_valid and fragment.structure_valid),
                )
                continue
    ...
    def _blocks_to_logical_chunks(
        ...
        *,
        ...
        def flush_text_group():
            ...
            content = "\n\n".join(
                str(block.content) for block in text_group if str(block.content).strip()
            )
            chunks = (
                [content]
        ...
        for block in blocks:
            ...
            if block.type is ParsedBlockType.LIST:
                logical_chunks.extend(
                    self._list_logical_chunks(block, token_num, delimiter, overlap)
                )
                continue
    ...
    def _split_parent_chunk(
        ...
        if block_type == ParsedBlockType.LIST.value:
            return [
                ...
                for chunk in self._split_list_content(
                    str(parent.content), token_num, delimiter, overlap
                )
                if chunk.strip()
            ]
    ...
    def _list_logical_chunks(
        ...
        return [
            ...
            for content in self._split_list_content(
                str(block.content), token_num, delimiter, overlap
            )
            if content.strip()
        ]
    ...
    def _split_strict_table(self, table, token_num: int) -> list[str]:
        ...
        if not table.find_all("tr"):
            ...
            section = next(
                (name for name in ("thead", "tbody", "tfoot") if table.find(name) is not None),
                None,
            )
            empty_table = (
                f"<table><{section}></{section}></table>" if section else "<table></table>"
            )
        ...
        if thead is not None:
            ...
            body_rows = [row for row in table.find_all("tr") if row.find_parent("thead") is None]
            if (
                body_rows
            ...
            section_rows = [("thead", row) for row in thead.find_all("tr")]
        ...
        if not chunks or any(
            not self._is_valid_table_content(chunk) or num_tokens_from_string(chunk) > token_num
            for chunk in chunks
        ):
            raise ValueError(f"Structured wrapper cannot fit within token limit {token_num}.")
        return chunks
    ...
    def _rowless_table_container(table) -> str | None:
        direct_text = "".join(
            str(child).strip() for child in table.children if getattr(child, "name", None) is None
        )
        containers = [
            child
    ...
    def _split_strict_table_row(
        ...
        for cell in cells:
            ...
            def wrap_cell(piece: str, cell_name: str = cell_name) -> str:
                row_fragment = f"<tr><{cell_name}>{escape(piece)}</{cell_name}></tr>"
    ...
    def _extract_table_parts(self, table) -> tuple[str, list[str]]:
        ...
        if thead is not None:
            header_html = str(thead)
            rows = [str(row) for row in table.find_all("tr") if row.find_parent("thead") is None]
    ...
    def _split_code_content(
        ...
        *,
        ...
        for line in body_lines:
            if (
                num_tokens_from_string(self._wrap_code_lines([line], fence_start, fence_end))
                > token_num
            ):
                if current_lines:
        ...
        if strict and any(num_tokens_from_string(piece) > token_num for piece in result):
            raise ValueError(f"Structured wrapper cannot fit within token limit {token_num}.")
        return result
    ...
    def _hard_split_wrapped(
        ...
        *,
        ...
        if strict and num_tokens_from_string(wrap("")) > limit:
            raise ValueError(f"Structured wrapper cannot fit within token limit {limit}.")
        ...
        while index < len(tokens):
            ...
            if boundary is None:
                if strict:
                    raise ValueError(f"Structured content cannot fit within token limit {limit}.")
                raise RuntimeError(
    ...
    def _decode_token_slice(tokens: list[int], start: int, end: int) -> str | None:
        try:
            return get_encoder().decode(tokens[start:end], errors="strict")
