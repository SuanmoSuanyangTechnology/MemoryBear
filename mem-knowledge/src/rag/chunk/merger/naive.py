...
from ..tokenization import naive_merge, naive_merge_docx, naive_merge_with_images
...
def build_parent_child_logical_chunks(
    ...
    if parent_chunk_mode == "full-doc":
        full_text = "\n\n".join(
            str(chunk.content) for chunk in parent_chunks if chunk.type == LogicalChunkType.TEXT
        )
...
class NaiveMerger(ChunkMerger):
    def merge(self, ctx: ChunkContext, parse_result: ParseResult) -> MergeResult:
        ...
        result = MergeResult(
            chunks=chunks, logical_chunks=logical_chunks, pdf_parser=parse_result.pdf_parser
        )
        if ctx.chunk_output_mode is ChunkOutputMode.PARENT_CHILD:
            ...
            parent_chunks = _text_logical_chunks(parent_texts) + _table_logical_chunks(
                parse_result.tables
            )
...
class DocxMerger(ChunkMerger):
    def merge(self, ctx: ChunkContext, parse_result: ParseResult) -> MergeResult:
        ...
        logical_chunks = _text_logical_chunks(chunks, images) + _table_logical_chunks(
            parse_result.tables
        )
        result = MergeResult(
            chunks=chunks,
        ...
        if ctx.chunk_output_mode is ChunkOutputMode.PARENT_CHILD:
            parent_token_num = int(ctx.parser_config.get("parent_chunk_token_num", 1024))
            parent_texts, parent_images = naive_merge_docx(
                parse_result.sections, parent_token_num, delimiter
            )
            parent_chunks = _text_logical_chunks(
                parent_texts, parent_images
            ) + _table_logical_chunks(parse_result.tables)
            child_chunks, parent_chunks, parent_id_map = build_parent_child_logical_chunks(
                parent_chunks,
                ctx.parser_config,
                lambda text, size: _split_text(text, size, delimiter),
            ...
            result.chunks = _legacy_chunks(child_chunks)
            result.images = [
                chunk.image for chunk in child_chunks if chunk.type is LogicalChunkType.TEXT
            ]
        return result
