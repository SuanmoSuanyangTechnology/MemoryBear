"""Parent-child result validation copied from the legacy chunk package."""
...
def validate_parent_child_result(
    ...
    for parent_index in range(len(parent_chunks)):
        if parent_index not in referenced_parent_indices:
            raise ValueError(
                f"Invalid {mode} hierarchy: parent index {parent_index} has no children."
            )


__all__ = ["GroupedChildChunks", "validate_parent_child_result"]
