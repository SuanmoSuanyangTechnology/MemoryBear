...
class HtmlParser(DocumentParser):
    def parse(self, ctx):
        sections, tables = self.parse_blocks(ctx.filename, ctx.binary)
        table_results = [
            ((None, table.get("content", "")), "") for table in tables if table.get("content")
        ]
        return [(section, "") for section in sections if section], table_results
    ...
    def read_text_recursively(
        cls, element, parser_result, chunk_token_num=512, parent_name=None, block_id=None
    ):
