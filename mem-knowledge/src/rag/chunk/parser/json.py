...
class JsonParser(DocumentParser):
    ...
    def parse(self, ctx):
        ...
        parser = type(self)(chunk_token_num)
        if ctx.binary is not None:
            content = ctx.binary.decode(find_codec(ctx.binary), errors="ignore")
        else:
            with open(ctx.filename) as file:
                content = file.read()
        sections = (
            parser._parse_jsonl(content)
            if parser.is_jsonl_format(content)
            else parser._parse_json(content)
        )
