...
### [Installation Guide](#memorybear安装教程)
...
Scalable, flexible storage: supportting millions of entities and tens of millions of relational edges, covering 12 core relationship types (hierarchical, causal, temporal, logical, etc.) to fit multi-domain knowledge applications.
Seamless integration with the extraction module: Extracting triples synchronize directly into Neo4j, automatically constructing the initial knowledge graph with zero manual mapping.
Interactive graph visualization: users can intuitively explore entity connection paths, adjust relationship weights, and perform hybrid “machine-generated + human-optimized” graph management.</br>
...
Dormancy – retained but with lower retrieval priority
Decay – gradually compressed to reduce storage cost
Clearance – permanently removed and archived into cold storage
This mechanism maintains redundant knowledge under 8%, reducing waste by over 60% compared with systems lacking forgetting capabilities.</br>
...
Follow the backend setup and startup steps described above.

step3:Start the Frontend Web Application
Install dependencies and start the web service as outlined previously.

step4:Initialize the Database and Generate the Super Admin Account
Run the following command in the terminal: curl.exe -X POST http://127.0.0.1:8000/api/setup .This initializes the database and creates the super administrator account.
...
Open the frontend URL displayed in the terminal after running npm run dev and log in using the admin credentials.
