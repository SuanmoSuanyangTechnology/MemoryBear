...
async def get_dedup_candidates_for_entities(  # 适配新版查询：使用全文索引按名称检索候选实体
    ...
    - 优先使用精确属性匹配（name + end_user_id）确保不受全文索引延迟影响；
    - 补充使用全文索引查询 `SEARCH_ENTITIES_BY_NAME` 召回模糊候选；
    ...
    async def _query_by_name(incoming: Dict[str, Any]) -> tuple[str, List[Dict[str, Any]]]:
        async with sem:
            ...
            # 1. 精确属性匹配（不受全文索引延迟影响，确保跨写入任务的同名实体能被召回）
            try:
            ...
            # 2. 全文索引补充召回（捕获模糊匹配候选，如别名、描述中包含该名称的实体）
            try:
