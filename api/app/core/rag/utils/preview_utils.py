...
def _build_normal_hierarchy(chunks: list[dict]) -> list[DocumentChunk]:
    ...
    return [
        DocumentChunk(
            ...
            metadata={**chunk, "chunk_type": chunk.get("chunk_type", "chunk")},
            children=[],
        )
        for chunk in chunks
    ]
    ...
    for parent_idx, parent_chunk in enumerate(parent_chunks):
        children = [
            ChildDocumentChunk(
                ...
                metadata={**child, "chunk_type": "child", "sort_id": child_idx},
            )
        ...
        result.append(
            DocumentChunk(
                ...
                metadata={**parent_chunk, "chunk_type": "parent", "sort_id": parent_idx},
                children=children,
            )
        )
    ...
    for idx in range(0, len(chunks), 2):
        ...
        if answer:
            children.append(
                ChildDocumentChunk(
                    ...
                    metadata={**answer, "chunk_type": "qa_answer"},
                )
            )
        ...
        result.append(
            DocumentChunk(
                ...
                metadata={**question, "chunk_type": "qa_question"},
                children=children,
            )
        )
    return result
