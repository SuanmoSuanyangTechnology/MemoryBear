...
from app.core.rag.deepdoc.vision import OCR, AscendLayoutRecognizer, LayoutRecognizer, Recognizer, TableStructureRecognizer
...
class RAGPdfParser:
    def __init__(self, **kwargs):
        ...
        try:
            model_dir = os.path.join(get_project_base_directory(), "res/text_concat_xgb_v1.0")
            self.updown_cnt_mdl.load_model(os.path.join(model_dir, "updown_concat_xgb.model"))
        except Exception:
            model_dir = snapshot_download(repo_id="InfiniFlow/text_concat_xgb_v1.0", local_dir=os.path.join(get_project_base_directory(), "res/text_concat_xgb_v1.0"), local_dir_use_symlinks=False)
            self.updown_cnt_mdl.load_model(os.path.join(model_dir, "updown_concat_xgb.model"))
    ...
    def __images__(self, fnm, zoomin=3, page_from=0, page_to=299, callback=None):
        ...
        try:
            ...
            logging.exception("RAGPdfParser __images__")
    ...
    def parse_into_bboxes(self, fnm, callback=None, zoomin=3):
        ...
        for b in self.boxes:
            ...
            b["positions"] = [[pos[0][-1] + 1, *pos[1:]] for pos in RAGPdfParser.extract_positions(b["position_tag"])]
...
class VisionParser(RAGPdfParser):
