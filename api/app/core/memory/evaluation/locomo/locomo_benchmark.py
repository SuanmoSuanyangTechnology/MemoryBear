...
async def run_locomo_benchmark(
    ...
    print(f"\n{'='*60}")
    print(f"🚀 Starting LoCoMo Benchmark Evaluation")
    print(f"{'='*60}")
    ...
    if skip_ingest:
        print(f"⏭️  Skipping data ingestion (using existing data in Neo4j)")
        print(f"   Group ID: {group_id}\n")
    else:
        print(f"💾 Checking database ingestion...")
        try:
            ...
            if success:
                ...
                print(f"⚠️  Ingestion may have failed, continuing anyway\n")
        
        except Exception as e:
            print(f"❌ Ingestion failed: {e}")
            print(f"⚠️  Continuing with evaluation (database may be empty)\n")
    ...
    try:
        ...
        print(f"📊 Printing results to console instead:\n")
