...
async def run_emotion_extraction(
    ...
    """对含 has_emotional_state=True 的 statements 进行情绪提取 + Neo4j 回填。

    提取逻辑：复用 EmotionExtractionStep（每条独立，asyncio.gather 并发），
    单条失败降级跳过，不影响其他 statement。

    Args:
        statements: [{"statement_id": ..., "statement_text": ..., "speaker": ...}]
        llm_model_id: LLM 模型 UUID
        language: zh / en
        emotion_config: 可选的 EmotionExtractionStep 配置覆盖
        snapshot_dir: 可选的快照目录路径
    ...
    context = StepContext(llm_client=llm_client, language=language, config=pipeline_config)
    ...
    async def _extract_one(stmt: dict):
        ...
        try:
            ...
            if snapshot_outputs is not None:
                snapshot_outputs[stmt["statement_id"]] = result.model_dump()
            extracted += 1
            logger.debug(
                f"[Emotion] 单条提取完成: stmt={stmt['statement_id']}, "
                f"type={result.emotion_type}, intensity={result.emotion_intensity}"
            )
        except Exception as e:
            failed += 1
            if snapshot_outputs is not None:
                snapshot_outputs[stmt["statement_id"]] = {"error": str(e)}
            logger.warning(
                f"[Emotion] 单条提取失败 stmt={stmt['statement_id']}: {e}"
            )

    await asyncio.gather(*[_extract_one(s) for s in statements])

    logger.info(
        f"[Emotion] 批量情绪提取完成: 提取={extracted}, 失败={failed}, "
        f"总数={len(statements)}"
    )
    ...
    if snapshot_outputs is not None and snapshot_dir:
        ...
        if upload_stage_snapshot(snapshot_dir, "4_emotion_outputs", snapshot_outputs):
            logger.info(
