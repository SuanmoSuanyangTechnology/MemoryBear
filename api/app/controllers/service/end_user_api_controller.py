...
async def create_end_user(
    ...
    db: Session = Depends(get_db),  # 由 check_end_user_quota 装饰器使用，函数体内用 get_db_context() 独立会话
...
async def get_end_user_mapping(
    ...
    with get_db_context() as db:
        ...
        # Serialize ORM objects before session closes (avoid DetachedInstanceError)
        user_items = [
            EndUserMappingItem(
