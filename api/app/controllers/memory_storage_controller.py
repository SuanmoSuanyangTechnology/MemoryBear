...
@router.post(
    ...
    responses={
        200: {
            "description": "SSE stream of extraction results",
...
async def get_hot_memory_tags_api(
    ...
    """
    获取热门记忆标签（带 Redis 缓存，异步版本）。

    缓存命中：仅 Redis 访问，完全异步。
    缓存未命中：异步查询数据库。
    """
    ...
    try:
        ...
        if cached_result:
            ...
            try:
                ...
                api_logger.warning("Failed to parse cached data, will refresh")
...
async def delete_end_user(
    ...
    """删除终端用户的全部记忆数据（Neo4j 节点 + PostgreSQL 记录，异步版本）。
    ...
    try:
        ...
        # 归属校验 + memory_count / 软删除共用同一 async session
        async with get_async_db_context() as db:
            ...
            try:
                await EndUserRepository(db).update_memory_count_async(end_user_id, 0)
                await EndUserRepository(db).soft_delete_by_end_user_id_async(end_user_id)
                await db.commit()
