...
async def resolve_config_id_async(config_id: UUID | int | str, db=None) -> UUID:
    ...
    Args:
        config_id: 配置ID（UUID、UUID字符串 或 整数）
        db: 可选的异步数据库会话（AsyncSession）。不传时内部自行开短 session。
...
async def _lookup_by_old_id_async(old_id: int, db) -> UUID:
    """异步版本：通过 config_id_old 查找 UUID。"""
    from sqlalchemy import select
    from app.models.memory_config_model import MemoryConfig

    async def _query(session) -> UUID:
        result = await session.execute(
            select(MemoryConfig).where(MemoryConfig.config_id_old == old_id)
        )
    ...
    # 未传 db，自己开一个短 async session
    from app.db import get_async_db_context
    async with get_async_db_context() as short_db:
        return await _query(short_db)
