...
单一问题：
{{"selected_agent_id": "agent_id", "confidence": 0.9, "need_collaboration": false}}

多个独立子问题（拆分）：
{{"selected_agent_id": "coordinator", "confidence": 0.9, "need_collaboration": true, "need_decomposition": true,
 "sub_questions": [{{"question": "子问题1", "agent_id": "agent1", "order": 1, "depends_on": []}}, ...],
 "collaboration_strategy": "decomposition"}}

多Agent协作：
{{"selected_agent_id": "agent_id", "confidence": 0.9, "need_collaboration": true,
 "collaboration_agents": [{{"agent_id": "agent1", "role": "primary", "order": 1}}, ...],
 "collaboration_strategy": "sequential"}}

注意：
- selected_agent_id 必须在上面列表中
- depends_on: 子问题依赖关系（如 [1] 表示依赖第1个）
- collaboration_strategy: decomposition/sequential/parallel/hierarchical
- 只做路由决策，不要回答问题内容
"""
    ...
    def _validate_decision(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if selected_agent_id not in self.sub_agents:
            ...
            decision["reasoning"] = f"原始选择无效，使用默认 Agent"
