...
class ConversationService:
    ...
    def _dispatch_memory_sync(
        ...
        """触发 sync_message 把消息同步到 memory_messages 表。
        ...
        try:
            import asyncio
            from app.core.memory.storage_services.extraction_engine.message_ingestion import sync_message
            ...
            def _on_task_done(task: asyncio.Task) -> None:
                ...
                if exc is not None:
                    logger.warning(
                        f"[ConversationService] sync_message 异步执行失败: "
            ...
            logger.warning(
                f"[ConversationService] sync_message 调度失败（不影响主流程）: "
