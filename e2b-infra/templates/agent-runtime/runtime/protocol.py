...
class EventType(str, Enum):
    ...
    AGENT_LOG = "agent_log"
    AGENT_LOG_FINAL = "agent_log_final"
...
class EventEmitter:
    ...
    def emit_agent_tool_start(self, tool_name, tool_input, step_id="", **kw):
        self.emit(EventType.AGENT_TOOL_START, {"step_id": step_id, "tool_name": tool_name, "tool_input": tool_input, **kw})

    def emit_agent_tool_end(self, tool_name, tool_output, step_id="", **kw):
        self.emit(EventType.AGENT_TOOL_END, {"step_id": step_id, "tool_name": tool_name, "tool_output": tool_output, **kw})

    def emit_agent_tool_error(self, tool_name, error, step_id="", **kw):
        self.emit(EventType.AGENT_TOOL_ERROR, {"step_id": step_id, "tool_name": tool_name, "error": error, **kw})
    ...
    def emit_agent_log(self, data, **kw):
        self.emit(EventType.AGENT_LOG, {"trace": data, **kw})

    def emit_agent_log_final(self, data, **kw):
        self.emit(EventType.AGENT_LOG_FINAL, {"trace": data, **kw})

    def emit_agent_thinking(self, content, **kw):
        self.emit(EventType.AGENT_THINKING, {"content": content, **kw})
