...
import { getChildNodeVariables, filterChildrenWithTypes } from './useVariableList'

/**
 * Build a `getFilteredVariableList` function bound to the current node context.
 * The returned function filters the available variable list based on the node
 * type and configuration key, mirroring the per-node-type rules.
 *
 * @param selectedNode - Currently selected node
 * @param graphRef - Reference to graph instance
 * @param variableList - Full variable suggestion list
 * @returns A function producing the filtered variable list
 */
export function useFilteredVariableList(
  selectedNode: Node,
   ...
   */
  const getFilteredVariableList = (nodeType?: string, key?: string): Suggestion[] => {
  ...
  return getFilteredVariableList;
}
