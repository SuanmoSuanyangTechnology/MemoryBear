...
>>>>>>> 5041e082 (feat(app): add application share permission management- Add database model for share permission- Add API endpoints for permission CRUD- Add service layer logic for permission validation- Update dependency libraries)
    ...
    appShare: {
      ...
      shareFailed: '共享失败',
      confirm: '确认共享',
      noWorkspace: '暂无可共享的工作空间',
      searchWorkspace: '搜索工作空间',      tabAll: '全部应用',
      tabSharedToMe: '共享应用',
      tabMyShared: '我的共享',
      ...
      switchPermission: '切换权限',
      permissionUpdated: '权限已更新',
      permissionUpdateFailed: '权限更新失败',
      active: '生效中',
      inactive: '未生效',
      noSharedOut: '暂无共享记录',
      noSharedToMe: '暂无共享给我的应用',
      sharedAppCount: '{{count}} 个应用共享到此空间',
      unshareAll: '全部取消',
      unshareAllTitle: '批量取消共享',
      confirmUnshareAll: '确定取消所有共享给「{{name}}」的应用？此操作不可恢复。',
      unshareAllSuccess: '已取消全部共享',
      confirmUnshare: '确认取消共享？对方将无法继续使用该应用',
      confirmRemoveShared: '确认从本空间移除该共享应用？此操作无法复原',
      removeAll: '全部取消',
      removeAllTitle: '批量取消共享应用',
      confirmRemoveAll: '确定从本空间移除来自「{{name}}」的所有共享应用？移除后将无法继续使用这些应用，此操作无法复原。',
      removeAllSuccess: '已移除全部共享应用',
      useShare: '使用共享',
      useShareDesc: '可测试运行，不可查看内部',
      copyShare: '复制共享',
      copyShareDesc: '复制完整副本，自由编辑',
      readonlyViewTip: '共享应用（只读）',
    },
  },
}