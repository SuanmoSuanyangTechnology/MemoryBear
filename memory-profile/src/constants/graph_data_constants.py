...
#
# 原为 ["caption"]，但 **caption 在整个图库中不存在**（实测 ``db.propertyKeys()``
# 无此键）——保留它会让每条引用该字段的 Cypher 都产生一条 Neo4j ``01N52``
# (property key does not exist) 警告，且实际返回恒为 null。故清空：
# 未列出的类型投影为空 map ``{}``，前端按"无属性"回落 label。
#
# 全类型共有字段实测为 created_at / end_user_id / id；三者都是标识或时间戳，
# 不适合作为"展示正文"兜底，故不用。若将来需要，先确认写入侧真的会写该字段。
...
NODE_PROPERTY_WHITELIST: dict[str, list[str]] = {
    ...
    "MemorySummary": ["summary", "content", "created_at"],
    "Perceptual": [
        "file_name",
        "file_path",
        "file_type",
    ...
    "Community": ["name", "summary", "created_at"],
}
